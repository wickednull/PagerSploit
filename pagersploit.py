...
def wifi_scan(band='abg', duration=20):
    ...
    try:
        ...
        # Query 2: best non-empty SSID per AP mac
        # pineapd stores named rows and unnamed rows separately — get the name separately
        ap_macs = tuple(r['mac'] for r in rows if r['mac'])
        ssid_map = {}   # mac -> best ssid string
        if ap_macs:
            ph = ','.join('?' * len(ap_macs))
            cur.execute(
                f"SELECT w.mac, s.ssid FROM ssid s "
                f"JOIN wifi_device w ON s.wifi_device=w.hash "
                f"WHERE s.type=4 AND s.ssid IS NOT NULL AND s.ssid != '' AND s.ssid != x'' "
                f"AND w.mac IN ({ph}) "
                f"GROUP BY w.mac",
                ap_macs
            )
