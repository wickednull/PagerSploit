...
# ── WiFi ──────────────────────────────────────────────────────────────────
# Encryption int -> string mapping from pineapd recon.db schema
RECON_DB = '/mmc/root/recon/recon.db'
# Encryption is a bitmask from nl80211 capability flags
# Bit 41 = WPA3 (SAE), Bit 34 = WPA2 (RSN), value 2 = WEP
_ENC_WPA3 = 1 << 41
_ENC_WPA2 = 1 << 34
...
def _decode_enc(v):
    ...
    if v == 2: return 'WEP'
    has3 = bool(v & _ENC_WPA3)
    has2 = bool(v & _ENC_WPA2)
    if has3 and has2: return 'WPA3 WPA2'
    if has3: return 'WPA3'
    if has2: return 'WPA2'
    return 'WPA2'  # non-zero fallback
...
def wifi_scan(band='abg', duration=20):
    ...
    log(f'WiFi scan from recon DB...')
    set_module('WiFi Scan', 'Reading recon DB...')

    if band == 'bg':   freq_filter = 'AND s.freq < 5000'
    elif band == 'a':   freq_filter = 'AND s.freq >= 5000 AND s.freq < 5950'
    elif band == '6':   freq_filter = 'AND s.freq >= 5950'
    else:               freq_filter = ''
    ...
    try:
        ...
        cur = con.cursor()
        # Use DB-relative timestamp — pineapd uses its own epoch, not system time
        cur.execute('SELECT MAX(time) FROM wifi_device')
        _max_t = cur.fetchone()[0] or 0
        cutoff = _max_t - max(duration * 10, 300)
        cur.execute(f"""
            SELECT s.ssid, w.mac, s.freq, s.signal, s.encryption,
                   s.channel, w.packets, s.hidden
            FROM ssid s
            JOIN wifi_device w ON s.wifi_device = w.hash
            WHERE s.type = 4 AND w.time >= {cutoff} {freq_filter}
            GROUP BY w.hash
        ...
        cur.execute(f"""
            ...
            WHERE s.type IN (1,2,3) AND w.time >= {cutoff}
            GROUP BY w.mac
        """)
        _rows = cur.fetchall()
        client_counts = dict((str(r['mac']), int(r['cnt'])) for r in _rows)
        con.close()
    ...
    for row in rows:
        ...
        seen.add(mac)
        mac_fmt = ':'.join(mac[i:i+2] for i in range(0,12,2)).upper() if len(mac)==12 else mac
        freq = row['freq'] or 0
        ch = row['channel'] or freq_to_channel(freq)
        ssid = row['ssid'] or ''
        if not ssid or row['hidden']: ssid = '<hidden>'
        enc = _decode_enc(row['encryption'])
        aps.append({'bssid':mac_fmt, 'ssid':ssid, 'channel':str(ch) if ch else '?',
            'freq':freq, 'signal':row['signal'] or -100, 'enc':enc, 'cipher':'',
            'clients':client_counts.get(mac,0), 'packets':row['packets'] or 0})

    aps.sort(key=lambda x: x['signal'], reverse=True)
    with STATE_LOCK: STATE['scan_results'] = aps[:80]
    fiveg = sum(1 for a in aps if a['freq'] > 5000)
    log(f'Scan done: {len(aps)} APs ({fiveg} on 5/6GHz)', 'success')
