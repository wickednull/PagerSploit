...
def wifi_scan(band='abg', duration=20):
    ...
    try:
        ...
        cur = con.cursor()
        # Get current scan ID — only show APs from the active recon session
        cur.execute('SELECT MAX(id) FROM scan')
        _scan_id = cur.fetchone()[0] or 1
        cur.execute(f"""
            SELECT s.ssid, w.mac, s.freq, s.signal, s.encryption,
                   s.channel, w.packets, s.hidden
            FROM ssid s
            JOIN wifi_device w ON s.wifi_device = w.hash
            WHERE s.type = 4 AND s.scan = {_scan_id} {freq_filter}
            GROUP BY w.hash
        ...
        cur.execute(f"""
            ...
            WHERE s.type IN (1,2,3) AND s.scan = {_scan_id}
            GROUP BY w.mac
        """)
        _rows = cur.fetchall()
