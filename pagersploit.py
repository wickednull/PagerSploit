...
def wifi_scan(band='abg', duration=20):
    ...
    # wlan0cli is the managed STA interface — can do iw scan while pineapd owns monitor ifaces
    for iface in ['wlan0cli', 'wlan0']:
        try:
            log(f'Trying iw dev {iface} scan...', 'info')
            ...
            if result.stdout.strip():
                out = result.stdout
                log(f'Got scan output from {iface}', 'info')
                break
