...
def wifi_scan(band='abg', duration=20):
    ...
    sql = (
        # Get the single highest-signal row per AP using a self-join
        # This is much faster than correlated subqueries on 20k rows
        "SELECT s.ssid, s.freq, s.signal, s.encryption, s.channel, w.mac, w.packets "
        "FROM ssid s "
        "JOIN wifi_device w ON s.wifi_device = w.hash "
        "WHERE s.type = 4 " + band_where + " "
        "AND s.signal = ("
        "  SELECT MAX(s2.signal) FROM ssid s2 WHERE s2.wifi_device = s.wifi_device AND s2.type = 4"
        ") "
        "GROUP BY w.mac "
    ...
    try:
        ...
        # Client count from type 1/2/3 rows
        cur.execute(
            "SELECT w.mac, count(*) as cnt "
            "FROM ssid s JOIN wifi_device w ON s.wifi_device = w.hash "
            "WHERE s.type IN (1,2,3) GROUP BY w.mac"
        )
    ...
    for row in rows:
        ...
        raw_ssid = row['ssid']
        try:
            ssid = bytes(raw_ssid).decode('utf-8', errors='replace').strip('\x00') if raw_ssid else ''
        except Exception:
        ...
        while not stop.is_set() and time.time() < deadline:
            try:
                ...
                for row in cur.fetchall():
                    ...
                    raw = row['ssid']
                    try:
                        ssid = bytes(raw).decode('utf-8', errors='replace').strip('\x00') if raw else ''
                    except:
                        ssid = str(raw or '')
                    if mac and ssid:
...
def wifi_wps_scan():
    ...
    try:
        ...
        if 'wps' in cols:
            ...
            for row in cur.fetchall():
                output.append({'bssid': _fmt_mac(str(row['mac'] or '')),
