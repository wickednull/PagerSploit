...
def wifi_scan(band='abg', duration=20):
    ...
    # Simple aggregation — one row per AP, best signal
    # No subqueries, no joins on 20k rows — fast on MIPS
    sql = (
        "SELECT w.mac, MAX(s.signal) AS signal, s.ssid, s.freq, s.encryption, s.channel, w.packets "
        "FROM ssid s "
    ...
    try:
        ...
        log(f'Running scan SQL...', 'info')
        cur.execute(sql)
        rows = cur.fetchall()
        log(f'Raw AP rows: {len(rows)}', 'info')

        # Client count — scoped to AP macs we found, avoids full table scan
    ...
    for row in rows:
        ...
        try:
            if isinstance(raw_ssid, (bytes, bytearray)):
                ssid = raw_ssid.decode('utf-8', errors='replace').strip('\x00').strip()
            elif raw_ssid:
