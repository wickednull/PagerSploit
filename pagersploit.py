...
async function api(path,method='GET',body=null){
  ...
  if(!r.ok&&r.status!==404)return{error:'HTTP '+r.status};
  return await r.json();}catch(e){console.error(path,e);return{error:e.message};}}
...
const TITLES={'dashboard':'OVERVIEW','wifi-scan':'AP SCANNER','wifi-deauth':'DEAUTH',
  ...
  'osint-http':'HTTP FINGERPRINT','osint-sysrecon':'SYS RECON','loot':'LOOT'};
function show(id){
...
function pollUntilIdle(cb,max=60){let n=0;const t=setInterval(async()=>{n++;const s=await api('/api/state');
  if(n>=max||(s&&s.active_module==='idle')){clearInterval(t);cb(s);}},1000);}
async function pollState(){
  ...
  if(lg&&!lg.error){const el=document.getElementById('log-entries');
