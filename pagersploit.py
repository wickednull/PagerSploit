...
def wifi_scan(band='abg', duration=20):
    """Read APs from pineapd recon.db. Confirmed schema from device:
    - ssid.bssid is always NULL -- use wifi_device.mac as the BSSID
    - ssid.ssid is bytes (may be empty) -- decode carefully
    - ssid.channel is NULL -- derive from ssid.freq via _freq_to_ch()
    - type=4 = AP beacon, type=5 = probe request
    - Join: ssid.wifi_device = wifi_device.hash
    """
    import sqlite3 as _sq
    log(f'WiFi scan from recon DB ({band})...')
    set_module('WiFi Scan', 'Reading recon DB...')

    if   band == 'bg': band_where = 'AND s.freq BETWEEN 2400 AND 2500'
    elif band == 'a':  band_where = 'AND s.freq BETWEEN 5000 AND 6000'
    elif band == '6':  band_where = 'AND s.freq > 6000'
    else:              band_where = ''
    ...
    try:
        ...
        log('Running scan SQL...', 'info')

        # Get current and recent scan IDs — prefer recent sessions over all history
        # This avoids showing 18k historical APs from everywhere the device has ever been
        cur.execute("SELECT id FROM scan ORDER BY id DESC LIMIT 10")
        recent_scan_ids = [r[0] for r in cur.fetchall()]
        latest_scan_id = recent_scan_ids[0] if recent_scan_ids else None

        # Try current session first, then expand to recent sessions if too few results
        scan_filter = ''
        if latest_scan_id:
            # Start with just the latest scan
            cur.execute(
                "SELECT count(DISTINCT wifi_device) FROM ssid WHERE type=4 AND scan=?",
                (latest_scan_id,)
            )
            latest_count = cur.fetchone()[0]
            if latest_count >= 10:
                # Current session has enough — use it only
                scan_filter = f"AND s.scan = {latest_scan_id}"
                log(f'Using current scan session {latest_scan_id} ({latest_count} APs)', 'info')
            elif len(recent_scan_ids) > 1:
                # Expand to last 10 sessions
                ids = ','.join(str(i) for i in recent_scan_ids)
                scan_filter = f"AND s.scan IN ({ids})"
                log(f'Using last {len(recent_scan_ids)} scan sessions ({latest_count} in latest)', 'info')
            else:
                scan_filter = ''
                log('Using full DB (single session)', 'info')

        # Query 1: best signal row per AP
        sql_signal = (
            "SELECT w.mac, MAX(s.signal) AS signal, s.freq, s.encryption, s.channel, w.packets "
            "FROM ssid s "
            "JOIN wifi_device w ON s.wifi_device = w.hash "
            "WHERE s.type = 4 " + band_where + " " + scan_filter + " "
            "GROUP BY w.mac "
            "ORDER BY MAX(s.signal) DESC "
            "LIMIT 200"
        )
        cur.execute(sql_signal)
        rows = cur.fetchall()
        log(f'Raw AP rows: {len(rows)}', 'info')

        # Query 2: ALL non-empty SSIDs for found APs — pick longest per mac in Python
        # Avoids correlated subqueries; fetches all candidates then deduplicates fast
        ap_macs = tuple(r['mac'] for r in rows if r['mac'])
        ssid_map = {}   # mac -> best (longest) ssid string
        if ap_macs:
            ph = ','.join('?' * len(ap_macs))
            cur.execute(
                f"SELECT w.mac, s.ssid FROM ssid s "
                f"JOIN wifi_device w ON s.wifi_device=w.hash "
                f"WHERE s.type=4 AND s.ssid IS NOT NULL "
                f"AND length(s.ssid) > 0 AND w.mac IN ({ph}) {scan_filter}",
                ap_macs
            )
            for r in cur.fetchall():
                ...
                if name and r['mac']:
                    key = str(r['mac'])
                    # Keep the longest SSID seen for this AP
                    if key not in ssid_map or len(name) > len(ssid_map[key]):
                        ssid_map[key] = name

        # Client count scoped to found APs
        client_counts = {}
        if ap_macs:
            ph = ','.join('?' * len(ap_macs))
            cur.execute(
                f"SELECT w.mac, count(*) as cnt FROM ssid s "
                f"JOIN wifi_device w ON s.wifi_device=w.hash "
                f"WHERE s.type IN (1,2,3) AND w.mac IN ({ph}) GROUP BY w.mac",
                ap_macs
            )
            client_counts = {str(r['mac']): int(r['cnt']) for r in cur.fetchall() if r['mac']}
        con.close()

    except Exception as e:
        log(f'Recon DB error: {e}', 'error')
    ...
    for row in rows:
        ...
        # Use dedicated SSID map (best non-empty name for this AP)
        ssid = ssid_map.get(str(mac), '')
        if not ssid:
            ssid = '<hidden>'
        ...
        cli = client_counts.get(str(mac), 0)

        aps.append({
            'ssid': ssid, 'bssid': bssid, 'channel': ch,
            'signal': sig, 'encryption': enc,
            'clients': cli, 'power': sig
        })
...
def _pineapple(*args):
    ...
    if _PINEAPPLE_SOURCE is not None:
        to_try = [_PINEAPPLE_SOURCE]
    else:
        to_try = sources
...
def wifi_probe_harvest(duration=60):
    """Read probe requests (type=5) directly from recon.db."""
    import sqlite3 as _sq
    stop = make_stop(); reg_stop('probe_harvest', stop)
    def _run():
        log('Probe harvest from recon DB', 'info')
        set_module('Probe Harvest', 'Reading...')
                ...
                for row in cur.fetchall():
                    ...
                    try:
                        if isinstance(raw, (bytes, bytearray)):
                            ssid = raw.decode('utf-8', errors='replace').strip('\x00').strip()
                        else:
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_DELETE(self):
        ...
        if path=='/api/loot/delete_all':
            ...
            log(f'Deleted all loot: {deleted} files','info')
