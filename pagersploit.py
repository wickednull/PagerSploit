...
def wifi_scan(band='abg', duration=20):
    ...
    def _fetch_pineap():
        """Try _pineap RECON APS via bash login shell. Strip Hak5 banner from stdout."""
        try:
            r = subprocess.run(
                ['bash', '--login', '-c', '_pineap RECON APS format=json limit=200'],
                capture_output=True, text=True, timeout=20
            )
            out = r.stdout
            # bash --login prints Hak5 banner to stdout before command output
            # Strip everything before the first '[' (start of JSON array)
            idx = out.find('[')
            if idx == -1:
                log(f'_pineap no JSON array in output (rc={r.returncode})', 'warn')
                return None
            out = out[idx:].strip()
            data = _json.loads(out)
            return data if isinstance(data, list) else data.get('aps', [])
        except _json.JSONDecodeError as e:
            log(f'_pineap JSON error: {e}', 'warn')
            return None
    ...
    if aps_raw is None:
        ...
        try:
            subprocess.run(['bash', '--login', '-c', '_pineap RECON NEW'], capture_output=True, timeout=10)
            time.sleep(3)
        except: pass
        aps_raw = _fetch_pineap()
    ...
    def _run():
        ...
        while not stop.is_set() and time.time() < deadline:
            try:
                ...
                idx = raw.find('[')
                if idx != -1:
                    clients = _json.loads(raw[idx:].strip())
