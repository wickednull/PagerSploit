...
def wifi_scan(band='abg', duration=20):
    """Scan APs using iw — same method as JackPoint which works correctly."""
    log(f'WiFi scan via iw ({band})...')
    set_module('WiFi Scan', 'Scanning...')

    aps = []
    out = ''

    # wlan0cli is the managed STA interface — can scan while pineapd owns monitor ifaces
    # Try wlan0cli first (most reliable), then wlan0 as fallback
    for iface in ['wlan0cli', 'wlan0']:
        try:
            log(f'Scanning via iw dev {iface}...', 'info')
            ...
            if result.stdout.strip():
                out = result.stdout
                log(f'iw scan done on {iface}', 'info')
                break
            elif result.stderr:
                log(f'iw {iface}: {result.stderr.strip()[:60]}', 'warn')
    ...
    if not out:
        log('iw scan failed on all interfaces', 'error')
        set_module('idle', '0 APs found')
        return []

    # Parse iw output — same logic as JackPoint
    current = {}
    for line in out.splitlines():
        ...
        if line.startswith('BSS '):
            ...
            try: current['channel'] = int(line.split('channel', 1)[1].strip())
            except: pass
        elif line.startswith('signal:'):
            try: current['signal'] = float(line.split('signal:', 1)[1].split()[0])
            except: pass
    ...
    # Band filter
    # Note: iw scan doesn't always give freq, use channel ranges
    if band == 'bg':
        aps = [a for a in aps if 1 <= a['channel'] <= 14]
    elif band == 'a':
        aps = [a for a in aps if a['channel'] > 14]
    elif band == '6':
        aps = [a for a in aps if a['channel'] > 180]

    # Sort by signal, mark hidden
