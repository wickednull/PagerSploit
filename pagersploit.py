...
def wifi_scan(band='abg', duration=20):
    """Fetch live APs from pineapd via _pineap RECON APS format=json.
    This is the correct API — returns SSIDs directly, no recon.db needed."""
    log(f'WiFi scan via _pineap RECON APS ({band})...')
    set_module('WiFi Scan', 'Querying pineapd...')

    band_freqs = {
        'bg':  lambda f: 2400 <= f <= 2500,
        'a':   lambda f: 5000 <= f <= 6000,
        '6':   lambda f: f >= 5925 and f <= 7125,
        'abg': lambda f: True,
    }
    ...
    try:
        ...
        if not result.stdout.strip():
            log(f'_pineap RECON APS returned no output (rc={result.returncode})', 'warn')
            set_module('idle', 'Scan failed')
            return []

        raw = _json.loads(result.stdout.strip())
        aps_list = raw if isinstance(raw, list) else raw.get('aps', [])
        log(f'Raw AP rows: {len(aps_list)}', 'info')

    except Exception as e:
        log(f'_pineap RECON APS error: {e}', 'error')
        set_module('idle', 'Scan failed')
        return []
    ...
    for ap in aps_list:
        ...
        if not freq_filter(freq):
            continue

        seen.add(mac)

        # SSID from beacon dict (keyed by hash)
        ssid = ''
        hidden = False
        channel = 0
        beacon = ap.get('beacon', {})
        if beacon:
            for beacon_data in beacon.values():
                if isinstance(beacon_data, dict):
                    ssid = beacon_data.get('ssid', '') or ''
                    hidden = beacon_data.get('hidden', False)
                    channel = int(beacon_data.get('channel', 0))
                    break
        # Fallback to response dict
        if not ssid:
            for resp_data in ap.get('response', {}).values():
                if isinstance(resp_data, dict):
                    ssid = resp_data.get('ssid', '') or ''
                    if ssid:
                        break

        if not ssid:
            ssid = '<hidden>' if hidden else '<hidden>'

        if not channel:
            channel = _freq_to_ch(freq)

        sig = int(ap.get('signal', -100))
        # _pineap RECON APS doesn't include encryption — check beacon data
