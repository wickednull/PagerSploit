...
def wifi_scan(band='abg', duration=20):
    """Fetch APs from pineapd. Uses _pineap RECON APS live API with recon.db fallback."""
    import json as _json
    log(f'WiFi scan via _pineap RECON APS ({band})...')
    set_module('WiFi Scan', 'Querying pineapd...')

    band_freqs = {
        'bg':  lambda f: 2400 <= f <= 2500,
        'a':   lambda f: 5000 <= f <= 5925,
        '6':   lambda f: 5925 <= f <= 7125,
        'abg': lambda f: True,
    }
    freq_filter = band_freqs.get(band, lambda f: True)
    pineap_env = {**os.environ}  # use full inherited env so _pineap (/usr/bin/_pineap) is found

    def _fetch_pineap():
        """Try _pineap RECON APS, return list or None on failure."""
        try:
            r = subprocess.run(
                ['_pineap', 'RECON', 'APS', 'format=json', 'limit=200'],
                capture_output=True, text=True, timeout=15, env=pineap_env
            )
            out = r.stdout.strip()
            if not out:
                return None
            data = _json.loads(out)
            return data if isinstance(data, list) else data.get('aps', [])
        except Exception as e:
            log(f'_pineap error: {e}', 'warn')
            return None

    def _fetch_recon_db():
        """Fallback: read recon.db directly."""
        import sqlite3 as _sq
        log('Falling back to recon.db...', 'warn')
        if not os.path.exists(RECON_DB):
            return []
        try:
            con = _sq.connect(f'file:{RECON_DB}?mode=ro', uri=True, timeout=10)
            con.row_factory = _sq.Row
            cur = con.cursor()
            cur.execute("PRAGMA query_only = ON")
            cur.execute("PRAGMA temp_store = MEMORY")
            # Get recent scan IDs
            cur.execute("SELECT id FROM scan ORDER BY id DESC LIMIT 10")
            scan_ids = [r[0] for r in cur.fetchall()]
            scan_filter = f"AND s.scan IN ({','.join(str(i) for i in scan_ids)})" if scan_ids else ""
            cur.execute(
                "SELECT w.mac, MAX(s.signal) AS signal, s.freq, s.encryption, s.channel, w.packets "
                f"FROM ssid s JOIN wifi_device w ON s.wifi_device = w.hash "
                f"WHERE s.type = 4 {scan_filter} "
                "GROUP BY w.mac ORDER BY MAX(s.signal) DESC LIMIT 200"
            )
            rows = cur.fetchall()
            ap_macs = tuple(r['mac'] for r in rows if r['mac'])
            ssid_map = {}
            if ap_macs:
                ph = ','.join('?' * len(ap_macs))
                cur.execute(
                    f"SELECT w.mac, s.ssid FROM ssid s "
                    f"JOIN wifi_device w ON s.wifi_device=w.hash "
                    f"WHERE s.type=4 AND s.ssid IS NOT NULL AND length(s.ssid)>0 AND w.mac IN ({ph})",
                    ap_macs
                )
                for r in cur.fetchall():
                    raw = r['ssid']
                    try:
                        name = (raw.decode('utf-8', errors='replace').strip('\x00').strip()
                                if isinstance(raw, (bytes, bytearray)) else str(raw).strip())
                    except:
                        name = ''
                    if name and r['mac']:
                        key = str(r['mac'])
                        if key not in ssid_map or len(name) > len(ssid_map[key]):
                            ssid_map[key] = name
            con.close()
            # Convert to same format as _pineap output
            result = []
            for row in rows:
                mac = _fmt_mac(row['mac'])
                if not mac: continue
                ssid = ssid_map.get(str(row['mac']), '')
                ch = int(row['channel'] or 0) or _freq_to_ch(row['freq'])
                result.append({
                    'mac': mac, 'signal': int(row['signal'] or -100),
                    'freq': int(row['freq'] or 0), 'channel': ch,
                    'ssid': ssid, 'encryption': row['encryption'],
                    '_from_db': True,
                })
            return result
        except Exception as e:
            log(f'recon.db fallback error: {e}', 'error')
            return []

    # Try _pineap first
    aps_raw = _fetch_pineap()

    # If empty, start recon and retry once
    if aps_raw is None:
        log('_pineap returned empty — starting new recon session...', 'warn')
        try:
            subprocess.run(['_pineap', 'RECON', 'NEW'], capture_output=True, timeout=5, env=pineap_env)
            time.sleep(3)
        except: pass
        aps_raw = _fetch_pineap()

    # If still empty, fall back to recon.db
    if aps_raw is None:
        aps_raw = _fetch_recon_db()
        if not aps_raw:
            log('No AP data from _pineap or recon.db', 'warn')
            set_module('idle', '0 APs found')
            return []

    log(f'Raw AP rows: {len(aps_raw)}', 'info')

    aps = []
    seen = set()
    for ap in aps_raw:
        # Handle both _pineap format and db fallback format
        if ap.get('_from_db'):
            bssid = ap['mac']
            ssid = ap['ssid'] or '<hidden>'
            channel = ap['channel']
            sig = ap['signal']
            enc = _enc_str(ap['encryption'])
            freq = ap['freq']
        else:
            mac = ap.get('mac', '').upper().strip()
            if not mac: continue
            bssid = _fmt_mac(mac)
            if not bssid: continue
            freq = int(ap.get('freq', 0))
            ssid = ''
            channel = 0
            hidden = False
            for bd in ap.get('beacon', {}).values():
                if isinstance(bd, dict):
                    ssid = bd.get('ssid', '') or ''
                    hidden = bd.get('hidden', False)
                    channel = int(bd.get('channel', 0))
                    break
            if not ssid:
                for bd in ap.get('response', {}).values():
                    if isinstance(bd, dict):
                        ssid = bd.get('ssid', '') or ''
                        if ssid: break
            if not ssid:
                ssid = '<hidden>'
            if not channel:
                channel = _freq_to_ch(freq)
            sig = int(ap.get('signal', -100))
            enc_val = ap.get('encryption')
            if enc_val is None:
                for bd in ap.get('beacon', {}).values():
                    if isinstance(bd, dict) and 'encryption' in bd:
                        enc_val = bd['encryption']; break
            enc = _enc_str(enc_val) if enc_val is not None else '?'

        if not freq_filter(freq) and freq > 0:
            continue
        if bssid in seen:
            continue
        seen.add(bssid)
        ...
        aps.append({
            ...
            'signal': sig, 'encryption': enc, 'clients': 0, 'power': sig
        })

    aps.sort(key=lambda x: x['signal'], reverse=True)
    with STATE_LOCK:
        STATE['scan_results'] = aps[:80]
...
def wifi_probe_harvest(duration=60):
    ...
    def _run():
        ...
        while not stop.is_set() and time.time() < deadline:
            try:
                r = subprocess.run(
                    ['_pineap', 'RECON', 'CLIENTS', 'format=json', 'limit=500'],
                    capture_output=True, text=True, timeout=10, env=pineap_env
                )
                if r.stdout.strip():
                    clients = _json.loads(r.stdout.strip())
                    if isinstance(clients, dict): clients = clients.get('clients', [])
                    for c in clients:
...
def lan_arp_scan(subnet='172.16.52.0/24'):
    ...
    # Scan requested subnet plus br-evil (evil twin clients) if not already included
    subnets = [subnet]
