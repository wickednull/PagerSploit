...
async function doCrack(){
  ...
  // Poll crack output from log
  pollUntilIdle(async(s)=>{
    if(out&&s)out.textContent=s.module_status||'Done';
    const loot=await api('/api/loot');
    const f=loot.find?.(l=>l.name.startsWith('crack_'));
...
async function macFromScan(){if(!scanResults.length)return alert('Run scan first');
  ...
  // Fire all requests, collect results
  const results=await Promise.all(bssids.map(async mac=>{
    const r=await api('/api/osint/mac','POST',{mac});
    if(r.error)return null;
    await new Promise(res=>setTimeout(res,300));
    const d=await api('/api/data/mac_lookup');
    return d?mac+': '+d.vendor:null;
  }));
  g('mac-result').textContent=results.filter(Boolean).join('\n')||'No vendors found';}
