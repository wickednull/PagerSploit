...
async function doCapture(){const b=gv('cap-bssid');if(!b)return alert('BSSID required');
  ...
  g('cap-status').textContent=r.error?'Error: '+r.error:'Capturing... handshakes → '+( r.handshake_dir||'/root/loot/handshakes');}
...
async function loadCrackSelects(){const loot=await api('/api/loot');
  const caps=loot.filter(l=>l.name.endsWith('.cap')||l.name.endsWith('.pcap')||l.name.endsWith('.22000')||l.name.endsWith('.hc22000'));
...
async function doSsh(){const t=gv('ssh-target');if(!t)return alert('Target required');
  ...
  pollUntilIdle(async(s)=>{
    ...
    const loot=await api('/api/loot');
    const f=loot.find?.(l=>l.name.startsWith('ssh_'+t));
    if(f){const r=await fetch('/api/loot/download/'+encodeURIComponent(f.name));
...
async function loadLoot(){const loot=await api('/api/loot');const tbody=g('loot-body');
  ...
  tbody.innerHTML=loot.map(l=>'<tr>'+
    ...
    '<td><button class="btn btn-red btn-sm" onclick="deleteLoot('+JSON.stringify(l.name)+')">Delete</button></td>'+
    '</tr>').join('');}
...
function termClear(){const el=termEl();if(el)el.innerHTML='';termLastLog=0;termLastTime='';}
...
function termInjectLog(entries){
  ...
  let startIdx=0;
  if(termLastTime){
    for(let i=entries.length-1;i>=0;i--){if(entries[i].time===termLastTime){startIdx=i+1;break;}}
  }
  const newEntries=entries.slice(startIdx);
  if(!newEntries.length)return;
  termLastTime=entries[entries.length-1].time;
